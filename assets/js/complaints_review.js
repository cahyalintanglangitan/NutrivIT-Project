...
async function loadComplaints() {
  ...
  const complaints = await apiRequest(`complaints_review.php?action=get_complaints&${queryString}`)
...
async function loadReviews() {
  ...
  const reviews = await apiRequest(`complaints_review.php?action=get_reviews&${queryString}`)
...
async function updateComplaintStatus(complaintId, newStatus) {
  ...
  const result = await apiRequest("complaints_review.php?action=update_complaint_status", {
    method: "POST",
    body: formData,
  })
...
function initializeCharts() {
  const analyticsData = window.analyticsData || {} // Declare analyticsData variable
  if (typeof analyticsData !== "undefined") {
    initComplaintTrendsChart(analyticsData)
    initComplaintTypesChart(analyticsData)
    initRatingDistributionChart(analyticsData)
    initTopReviewedProductsChart(analyticsData)
  }
}

function initComplaintTrendsChart(analyticsData) {
...
function initComplaintTypesChart(analyticsData) {
...
function initRatingDistributionChart(analyticsData) {
...
function initTopReviewedProductsChart(analyticsData) {
